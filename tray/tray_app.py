...
Architecture:
  ...
  Thread 3      → Flask API
  Thread 4      → Folder watcher (watchdog)
...
class TrayApp:
    ...
    def start(self):
        ...
        # Load CLIP once on main thread before anything starts
        self._load_clip()
        ...
        # Build tkinter window (must be on main thread)
        ...
        # Run tkinter main loop on main thread (blocks here)
    ...
    def _build_search_window(self):
        ...
        self.search_window.hide()   # hidden until tray click

    def _start_tray_thread(self):
        """Run pystray in its own thread so tkinter owns the main thread."""
        t = threading.Thread(
    ...
    def _start_scan(self, auto=False):
        ...
        self.scanner  = FullSystemScanner(
            embedder=self.embedder,
    ...
    def _open_search(self, icon=None, item=None):
        """Show search window — must schedule on main thread via after()."""
    ...
    def _show_stats(self, icon=None, item=None):
        ...
        repo  = MediaRepository()
        total = repo.count()
        emb   = repo.count_embedded()
        scan  = " (scanning...)" if self._scanning else ""
        self._notify("Stats", f"Total: {total}  |  Indexed: {emb}{scan}")
    ...
    def _on_scan_complete(self, inserted, skipped):
        ...
        self._notify("Scan Complete",
                     f"{inserted} new images indexed, {skipped} skipped.")
